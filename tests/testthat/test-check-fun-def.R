...
test_that("check_fun_def with ellipsis", {
  expect_equal(TRUE, TRUE)
  lst <- list()
  lst$DC_SOLUTION <- "f <- function(a, b=1, ...) sum(a + b, ...)"
  lst$DC_SCT <- "fundef <- ex() %>% check_fun_def('f') %>% check_body() %>% check_function('sum', index=1)"
  lst$DC_CODE <- lst$DC_SOLUTION
  capture.output(output <- test_it(lst))
  passes(output)
})